...
"""
func parseGerund() -> [Rule] {
    var i = 0
    
    let gerundRules = gerundStr
    ...
    let infinitiveRules = infinitiveStr
        .components(separatedBy: "\n")
        .map { s -> Rule in
            let row = s.components(separatedBy: "    ")
            i = i + 1
    ...
    return gerundRules + infinitiveRules
}
